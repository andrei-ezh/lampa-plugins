(function () {
  ...
  function changeParser() {
    var id = Lampa.Storage.get("lme_url_two");
    var selected = parsersInfo.find(p => p.base === id);

    if (!selected) return;

    var s = selected.settings;

    Lampa.Storage.set(s.parser_torrent_type === 'prowlarr' ? "prowlarr_url" : "jackett_url", s.url);
    Lampa.Storage.set(s.parser_torrent_type === 'prowlarr' ? "prowlarr_key" : "jackett_key", s.key);
  ...
  function startPlugin() {
    if (window.plugin_lmepublictorr_ready) return;
